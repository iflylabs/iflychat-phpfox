...
private function _get_ext_d_i() {
    ...
    if($this->_verify_access()) {
      $my_settings = array(
	      ...
	      'searchBar' => (Phpfox::getParam('iflychat.enable_search_bar'))?'1':'2',
              ...
              'chat_type' => '2',
              'iflychat_app_id' => Phpfox::getParam('iflychat.app_id'),
              'iflychat_external_cdn_host' => 'cdn.iflychat.com'
		  );
      if($this->_get_path_visibility() == '1'){
      	$my_settings['visible'] = '1';
      }else if($this->_get_path_visibility() == '2' && !Phpfox::isAdmin()){
      	$my_settings['visible'] = '1';
      }else if(($this->_get_path_visibility() == '3' || $this->_get_path_visibility() == '4') && $this->pathCheck()){
      	$my_settings['visible'] = '1';
      }else{
 ...
 //    }
	...
	private function iflychat_check_chat_admin()
	{
	    ...
	    $a = Phpfox::getParam('iflychat.admin_list');
	    if (!empty($a)) {
	        $a_names = explode(",", $a);
	        foreach ($a_names as $an) {
	            $aa = trim($an);
	            if ($aa == $this->_get_user_username()) {
	                return TRUE;
	                break;
	            }
	        }
	...
	{
	    $a = Phpfox::getParam('iflychat.moderartor_list');
	...
	private function _get_user_auth(){
		if($this->iflychat_check_chat_admin()){
			$chat_role = "admin";
		}elseif ($this->iflychat_check_chat_moderator()) {
			$chat_role = "moderator";
		}else{
		...
		if(Phpfox::getParam('iflychat.display_name') == "Display Name"){
			$user_data['user_name'] = $this->_get_user_name();
		}elseif (Phpfox::getParam('iflychat.display_name') == "Username") {
			$user_data['user_name'] = $this->_get_user_username();
		}else{
			$user_data['user_name'] = $this->_get_user_name();
		}

		$user_data['user_avatar_url'] = $this->_return_pic_url();
		$user_data['user_profile_url'] = Phpfox::getLib('url')->makeUrl('profile', $this->_get_user_username());
		...
		if(Phpfox::getParam('iflychat.enable_friends')) {
		  ...
		  if($chat_role == 'admin' || $chat_role == 'moderator') {
		  	$user_data['user_site_roles'] = $this->getUserRoles();
		  }
		  return $user_data;
	}
	private function _get_auth()
	{
//old code commented from here
// 		if(Phpfox::isAdmin()) {
// 			$role = "admin";
// 		}
// 		else {
...
//till here
  ...
  */
  ...
  $data = array(
    'api_key' => Phpfox::getParam('iflychat.api_key'),
    'app_id' => Phpfox::getParam('iflychat.app_id')
  );
...
//     );
    ...
    if(!empty($key)) {
      // $result = $this->_iflychat_extended_http_request($this->_get_external_a_host() . ':' . $this->_EXTERNAL_A_PORT .  '/z/', $options);
    	$result = (array)$this->_get_auth();
      if($_SESSION['code'] != 200) {
	      	if($_SESSION['code'] == 403){
	      		Phpfox_Error::set("Unable to connect to Iflychat server.Error Message - Invalid API Key.");
	      	}else if($_SESSION['code'] == 503){
	      		Phpfox_Error::set("Unable to connect to Iflychat server.503 Error. Service Unavailable.");
	      	}else{
	      		Phpfox_Error::set("Unable to connect to Iflychat server. Error code - " . $_SESSION['code'] . ".");
	      	}
	    }
    }
    if(!$this->iflychat_validate_fields()){
    	Phpfox_Error::set("Invalid App ID");
    }
  }
...
private function _get_path_visibility() {
    $v = Phpfox::getParam('iflychat.popup_chat');
    if($v == "Everywhere"){
    	return '1';
    }elseif($v == "Frontend Only"){
    	return '2';
    }elseif ($v == "Everywhere except those listed") {
    	return '3';
    }elseif ($v == "Only the listed pages") {
    	return '4';
    }elseif ($v == "Disable") {
    	return '5';
    }else{
  ...
  private function iflychat_validate_fields(){
  	$app_id = Phpfox::getParam('iflychat.app_id');
